...
addpath(genpath('../../../Libraries/ieeg-matlab-1.13.2-nomongo'));
...
load 'szMdl.mat';
for subjIdx = 7:numel(session.data)
    ...
    parfor i = 1:numSegs
        ...
        if exist(parsavename,'file') ~= 2
            fprintf('%s - Working on segment %g of %g...\n',datasetName,i,numSegs);
            ...
            fprintf('%s - Segment %d exists, skipping...\n',datasetName,i);
        end
    end
    %load all parFeats
    fn = dir(sprintf('tmp/%s-scores-par*',datasetName));
    fn = {fn.name};
    preds = [];

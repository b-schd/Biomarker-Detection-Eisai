...
% default: run('LL','SVM','test',5,1,1,'append')
% Seizure detection
...
f_Y = [];
szdurations = [];
for i = 1:numel(session.data) % for al
    ...
    if ~isempty(feat) && ~isempty(feat2)
        ...
        if indivMode==1
            ...
            run_detections(session.data(i),mdl,winLen,winDisp,durationThreshold,minThreshold,mergeThreshold,ch{1},featFn,layer_prefix,feature_prefix,'indiv','overwrite')
        end
    else

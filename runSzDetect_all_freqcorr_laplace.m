...
for subjIdx = 1:numel(session.data)
    ...
    parfor i = 1:numSegs
        parsavename = sprintf('tmp2/%s-scores-par%0.3d.mat',datasetName,i);
        if exist(parsavename,'file') ~= 2
            ...
            if count == 10
                ...
                % line up
                    ...
                    tmpfeats{j} = calc_featureswithfreqcorr(clip,fs);
   ...
   % final_preds = cell2mat(preds);
    ...
    uploadAnnotations(session.data(subjIdx),'Detected SZ-FC',szTimes,eventChannels,'SZ',0)
end
